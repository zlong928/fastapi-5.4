...
import { getPapers, uploadDocument } from "@/lib/api";
...
export function PapersPage() {
  ...
  const uploadMutation = useMutation({
    mutationFn: (target: File) => uploadDocument(target, undefined, "auto"),
    onSuccess: (response) => {
      ...
      <section className="rounded-3xl border border-slate-100 bg-white p-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">论文会通过现有文档上传入口入库；仅支持 PDF 文件</p>
          </div>
